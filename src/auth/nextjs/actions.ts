...
const data = result.data; // Now data is guaranteed to be valid
  ...
  try {
    ...
    const [user] = await db.insert(UserTable).values({name: data.name,email: data.email,password: hashedPassword,salt,}).returning({ id: UserTable.id, role: UserTable.role })

    if (user == null) return "Unable to create account2"


    await createUserSession(user, await cookies())//causing error
